...
# Hidden condition: each cell can assume at most one of the 4 proposed colors
# Hidden condition: extremes should have at least one neighbor cell with the same color
...
for i in range(5):
	for j in range(5):
		...
		if right:
			l.append(Equals(grid[i][j], grid[i][j+1]))
		if problem[i][j] == 0:
			# internal value: the value is not provided
			# the cell can be one of the 4 colors (1/2/3/4)
			solver.add_assertion(And(GE(grid[i][j], Int(1)), LE(grid[i][j], Int(4))))
			# each cell must have at least two neighbours of the same color (internal value)
			l2 = [Ite(x, Int(1), Int(0)) for x in l]
			solver.add_assertion(GE(Plus(l2), Int(2)))
		else:
			# external value: the value is provided in the input
			solver.add_assertion(Equals(grid[i][j], Int(problem[i][j])))
			# at least one neighbout must be of the same color (external value)
